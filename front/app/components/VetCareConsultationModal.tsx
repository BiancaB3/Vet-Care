...
import type { ConsultationDraft } from '../context/DraftContext';
    ...
    <div className="bg-white rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-slate-200 shadow-2xl">
      ...
      <form onSubmit={onSubmit} className="p-6 space-y-5">
        ...
        <div>
          ...
          <textarea value={consultationForm.diagnosis} onChange={(e) => onConsultationFormChange({ ...consultationForm, diagnosis: e.target.value })} rows={3} placeholder="Descreva o diagnóstico" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-primary font-medium resize-none" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-2">Prescrição / Recomendações</label>
          <textarea value={consultationForm.prescription} onChange={(e) => onConsultationFormChange({ ...consultationForm, prescription: e.target.value })} rows={3} placeholder="Medicamentos, dosagem, recomendações..." className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-primary font-medium resize-none" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-2">Observações Gerais</label>
          <textarea value={consultationForm.notes} onChange={(e) => onConsultationFormChange({ ...consultationForm, notes: e.target.value })} rows={2} placeholder="Observações adicionais" className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-primary font-medium resize-none" />
        </div>
